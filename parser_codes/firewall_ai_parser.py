"""
Project Compass — Firewall Log AI Access Detector
==================================================
Parses generic firewall CSV logs and flags traffic to known AI-related
destinations using the Azure SQL domain registry or the built-in ruleset.

Supported column format (auto-detected, header row required):
  timestamp, src_ip, src_port, dst_ip, dst_port, protocol,
  action, application, url, bytes_sent, bytes_received, duration, user

Usage:
  python3 firewall_ai_parser.py sample_firewall.csv --output-dir reports
  python3 firewall_ai_parser.py sample_firewall.csv --min-risk medium
  python3 firewall_ai_parser.py sample_firewall.csv --use-azure-registry
  python3 firewall_ai_parser.py sample_firewall.csv --summary-only
"""
...
# Log parser
# ---------------------------------------------------------------------------
def parse_firewall_log(filepath, ruleset, min_risk="low"):
    """
    Parse a firewall CSV log and return:
    ...
    col_map = None  # Will be set from header row
    ...
    with open(filepath, newline="", encoding="utf-8") as f:
        for lineno, raw in enumerate(f, start=1):
            ...
            if col_map is None:
                if line.lower().startswith("timestamp") or "src_ip" in line.lower():
                    headers = [h.strip().lower() for h in row]
                    col_map = {h: i for i, h in enumerate(headers)}
                    continue
                else:
                    # No header found — use positional defaults
                    col_map = {
                        "timestamp": 0, "src_ip": 1, "src_port": 2,
                        "dst_ip": 3, "dst_port": 4, "protocol": 5,
                        "action": 6, "application": 7, "url": 8,
                        "bytes_sent": 9, "bytes_received": 10,
                        "duration": 11, "user": 12
                    }
            ...
            bytes_received = get("bytes_received", "0")
            duration       = get("duration", "0")
            user           = get("user", src_ip)

            # Try URL first, then application field for domain matching
            domain = extract_domain_from_url(url_field)
            if not domain:
                domain = application.lower().strip() if application else ""
            ...
            # Parse byte counts safely
            try:
                b_sent = int(bytes_sent) if bytes_sent.isdigit() else 0
            except Exception:
                b_sent = 0
            try:
                b_recv = int(bytes_received) if bytes_received.isdigit() else 0
            except Exception:
                b_recv = 0
...
def build_summary(flagged):
    ...
    for e in flagged:
        ...
        app    = e["application"] if e["application"] else domain
        ...
        by_domain[domain]["bytes"]  += e["bytes_sent"] + e["bytes_received"]
        by_domain[domain]["actions"].add(action)

        # by user/IP
        if user not in by_user:
            by_user[user] = {"domains": set(), "count": 0, "bytes": 0}
        by_user[user]["domains"].add(domain)
        by_user[user]["count"]  += 1
        by_user[user]["bytes"]  += e["bytes_sent"] + e["bytes_received"]
        ...
        total_bytes += e["bytes_sent"] + e["bytes_received"]
...
def print_terminal_summary(flagged, summary, errors, skipped, log_file):
    ...
    print(f"\n{sep}")
    print(f"  Project Compass — Firewall AI Access Detection Report")
    ...
    # Risk breakdown with colored squares
    print(f"\n--- Risk Breakdown ---")
    risk_colors = {
        "high":    "\033[91m",
        "medium":  "\033[93m",
    ...
    # Top AI destinations
    print(f"\n--- Top AI Destinations ---")
    print(f"  {'Domain':<35} {'Category':<22} {'Risk':<10} {'Hits':>6} {'Data':>10}")
    print(f"  {'-'*35} {'-'*22} {'-'*10} {'-'*6} {'-'*10}")
    for domain, info in list(summary["by_domain"].items())[:10]:
        risk   = info["risk"]
        color  = risk_colors.get(risk, "")
        print(
            f"  {domain:<35} {info['category']:<22} "
            f"{color}{risk:<10}{reset} {info['count']:>6} "
            f"{format_bytes(info['bytes']):>10}"
        )

    # Top users/IPs
    print(f"\n--- Users / Source IPs with AI Traffic ---")
    print(f"  {'User / IP':<18} {'Unique Domains':>15} {'Total Hits':>12} {'Total Data':>12}")
    print(f"  {'-'*18} {'-'*15} {'-'*12} {'-'*12}")
    for user, info in list(summary["by_user"].items())[:10]:
        print(
            f"  {user:<18} {len(info['domains']):>15} "
            f"{info['count']:>12} {format_bytes(info['bytes']):>12}"
        )
    ...
    print(f"\n--- Report Summary ---")
    print(f"  This report identifies firewall traffic to AI-related destinations.")
    print(f"  {len(flagged)} of {total + len(errors)} total entries matched known AI services,")
    print(f"  spanning {len(summary['by_domain'])} unique destinations and "
          f"{len(summary['by_user'])} unique users/IPs.")
    print(f"  Total data transferred to AI services: {format_bytes(summary['total_bytes'])}")
    print(f"  Risk levels reflect destination sensitivity.")
...
def write_csv(flagged, summary, output_dir, stem):
    ...
    with open(domain_path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["domain", "category", "risk_level", "hit_count", "total_bytes", "actions"])
        for domain, info in summary["by_domain"].items():
            w.writerow([
                domain, info["category"], info["risk"],
                info["count"], info["bytes"], "|".join(info["actions"])
            ])
    ...
    with open(user_path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["user_or_ip", "unique_domains", "total_hits", "total_bytes"])
        for user, info in summary["by_user"].items():
            w.writerow([user, len(info["domains"]), info["count"], info["bytes"]])
...
def main():
    parser = argparse.ArgumentParser(
        description="Project Compass — Firewall Log AI Access Detector"
    )
    parser.add_argument("log_file",
        help="Path to the firewall CSV log file")
    ...
    # Parse
    flagged, skipped, errors = parse_firewall_log(
